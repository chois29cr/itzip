...
<table style="margin-left: auto; margin-right: auto;" border="0" cellpadding="0" cellspacing="0" width="1200px" align="center">
	<tr>
		<td>
			<table border="0" width="100%">
				...
				<tr>					
					<td width="25%" align="center"><a class="cur_active" href="/teamc2/manager/oArticle.do?page=1">게시물관리</a></td>
		...
		<td>			
			...
			<table style="margin-left: auto; margin-right: auto;" border="0" class="boardtable" cellpadding="0" cellspacing="0" width="100%">
				...
				for(int i=0; i< List.size(); i++) {
					ManagerVO vo = (ManagerVO)List.get(i);
					int seqno = total - ((Integer.parseInt(pageno) - 1) * 10);
					String title = vo.getTitle();
					if(title.length() >70){						
						title  = "<span title='"+title+"'>" + title.substring(0,70);
						title += "<font color=#ff6600>...</font></span>";
					}
				%>
				<tr>
	...
	<tr>
		<td align="center">	
			...
			%>
				<a class="page_nation" href="oArticle.do?page=<%= Integer.parseInt(pageno) - 1 %>">이전</a>&nbsp;
			<%
			}else {
			%>
			...
			for(int y=startno; y <= endno; y++) {
				...
				%>
					<a class="active" href="oArticle.do?page=<%= y %>"><%= y %></a>&nbsp;<%
				}else {
				%>
					<a class="page_nation" href="oArticle.do?page=<%= y %>"><%= y %></a>&nbsp;<%								
				}
			}
			
			if( Integer.parseInt(pageno) < maxpageno) {
			%>
				<a class="page_nation" href="oArticle.do?page=<%= Integer.parseInt(pageno) + 1 %>">다음</a>
			<%
			}else {
			%>
