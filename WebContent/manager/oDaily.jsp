...
<%
ArrayList<ClassboardVO> cList = (ArrayList<ClassboardVO>)request.getAttribute("cList");
...
<table style="margin-left: auto; margin-right: auto;" border="0" cellpadding="0" cellspacing="0" width="1200px" align="center">
		...
		<td>
			<table border="0" width="100%">
				...
				<tr>						
					...
					<td width="25%" align="center"><a class="cur_active" href="/teamc2/manager/oDaily.do?page=1">클래스관리</a></td>
		...
		<td>			
			...
			<table style="margin-left: auto; margin-right: auto;" border="0" class="boardtable" cellpadding="0" cellspacing="0" width="100%">
				...
				for(int i=0; i< cList.size(); i++) {
					ClassboardVO vo = (ClassboardVO)cList.get(i);
					int seqno = total - ((Integer.parseInt(pageno) - 1) * 10);
				%>
				<tr>
					<td class="line"height="40px" align="center">
						<input type="checkbox" id="no" name="no" value="<%= vo.getType() %>:<%= vo.getCno() %>" >						
					</td>
					...
					<td class="line" align="left">
						<a class="daily" href="/teamc2/manager/oStudent.do?page=1&cno=<%= vo.getCno() %>"> <%= vo.getTitle() %> 
						<input class="classTitle" id="title" name="title" type="hidden" value="<%= vo.getTitle() %>">
						</a>
					</td>
					<td class="line" align="center">
						<%= vo.getName() %>
	...
	<tr>
		<td align="center">	
			...
			%>
				<a class="page_nation" href="oDaily.do?page=<%= Integer.parseInt(pageno) - 1 %>">이전</a>&nbsp;
			<%
			}else {
			%>
			...
			for(int y=startno; y <= endno; y++) {
				...
				%>
					<a class="active" href="oDaily.do?page=<%= y %>"><%= y %></a>&nbsp;<%
				}else {
				%>
					<a class="page_nation" href="oDaily.do?page=<%= y %>"><%= y %></a>&nbsp;<%								
				}
			}
			
			if( Integer.parseInt(pageno) < maxpageno) {
			%>
				<a class="page_nation" href="oDaily.do?page=<%= Integer.parseInt(pageno) + 1 %>">다음</a>
			<%
			}else {
			%>
