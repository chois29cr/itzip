...
ArrayList<MypageVO> myreplylist = (ArrayList<MypageVO>)request.getAttribute("myreplylist");
...
<table style="margin-left: auto; margin-right: auto;" cellpadding="0" cellspacing="0" width="1000px" align="center">
			...
			<table border="0" width="100%">
				...
				<tr>
					<td colspan="2" style="padding:5px;">
						...
						<a class="cur_active" href="../mypage/myreply.do">내 댓글</a> &nbsp;&nbsp;&nbsp;
				...
				<tr height="300px">
					<td colspan="3" class="mytd" style="vertical-align:top;">
						...
						%>
							...
							<tr align="center">
								<td style="padding:20px 0 100px 0;"> 작성한 댓글이 없습니다. </td>
							</tr>
						<%
						}
						//mypageVO 에서 -> 내가 쓴 댓글 (마이룸, 큐앤에이, 클래스) 불러오기
						for(int i=0; i < myreplylist.size() ; i++){
							MypageVO m = myreplylist.get(i);
						...
						%>
							<tr class="mypost">
								...
								<td align="left" style="padding:0 0 0 10px;"onclick="location.href='../02class/view.do?cno=<%= m.getNo() %>&k=<%= k %>&t=<%= t %>&page=<%= ipageno %>'">
									<%=m.getTitle() %>
								</td>
								<td width="150px" onclick="location.href='../02class/view.do?cno=<%= m.getNo() %>&k=<%= k %>&t=<%= t %>&page=<%= ipageno %>'">
									<%=m.getDate() %>
								</td>			
							</tr>
						<%
	...
	<tr>
		<td align="center">	
			...
			if( ipageno > 1) {
			%>
				<a class="page_nation" href="myreply.do?page=<%= ipageno - 1 %>">이전</a>&nbsp;
			<%
			}else {
			%>
			...
			for(int y=startno; y <= endno; y++) {
				if(y==ipageno) {
			%>
					<a class="active" href="myreply.do?page=<%= y %>"><%= y %></a>&nbsp;
				<%
				}else {
				%>
					<a class="page_nation" href="myreply.do?page=<%= y %>"><%= y %></a>&nbsp;
				<%								
				}
			}
			...
			%>
				<a class="page_nation" href="myreply.do?page=<%= ipageno + 1 %>">다음</a>
			<%
			}else {
			%>
...
function doDelete(){
	...
	if(noList == ""){
		alert("삭제 할 댓글을 선택하세요.");
		return;
	}
	if( confirm("정말로 삭제하시겠습니까?") != 1){
		return false;		
	}
	document.location = "rdelete.do?no=" + noList +"&page=<%= ipageno %>";	
}
</script>
